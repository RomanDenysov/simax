			...
			<div className='w-full mx-auto flex flex-col items-center justify-center gap-y-10 border-2 rounded-xl px-4 py-5'>
				...
				<ul className='grid lg:grid-cols-2 lg:px-6 gap-2 '>
					{dummyData.map(({ id, text }) => (
						<li
							key={`item-${id}`}
							className='flex items-start justify-start gap-x-4 size-fit'
						>
							<Icons.achive className='size-[55px] min-h-[55px] min-w-[55px]' />
							<span className='text-base pt-2 md:text-lg xl:text-xl font-medium'>
								{text}
							</span>
						</li>
					))}
