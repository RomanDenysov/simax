			...
			>
				...
				<ul className='grid lg:grid-cols-2  lg:px-6 gap-2 '>
					{data.map(({ id, text }) => {
						...
						return (
							<AnimatedWords delay={delay} isVisible={isVisible}><Icons.achive className='size-[55px] min-h-[55px] min-w-[55px] self-start' />
			<span className='text-base md:text-lg xl:text-xl font-medium'>{text}</span></AnimatedWords>;
						)
					})}
				</ul>
			</div>
