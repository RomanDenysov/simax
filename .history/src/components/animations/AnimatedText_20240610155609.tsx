...
const AnimatedByCharacters = ({ children, delay}: AnimatedTextProps) => {
	return <motion.span initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			transition={{ delay: delay, duration: 0.5 }} className="text-3xl sm:text-6xl md:text-8xl w-full font-bold">{children}</motion.span>;
};
...
const AnimatedText = ({ text }: TitleProps) => {
	...
	return (
		<motion.h1
			className={cn(`text-3xl max-w-full font-bold flex flex-wrap items-center justify-center gap-x-2 w-full`)}
			ref={element}
		>
